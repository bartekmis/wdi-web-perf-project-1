...
import { getButtonConnection, getButtonVariant, getContainerSize, getHeadlineSize, getSectionSettings } from '@/lib/theme-utils';
import { SectionSettings } from '@/types/theme';
import Headline from '@/components/Components/Headline';

const Section25x25x25x25 = ({ data }: { data: any }) => {
  ...
  return (
    ...
    >
      ...
      <div className={getContainerSize(sectionSettings.containerSize)}>
        ...
        <div className='content grid gap-12 lg:grid-cols-4'>
          {data.column1_text && (
            <div className='flex flex-col'>
              <div className='content animation-fadeIn' dangerouslySetInnerHTML={{ __html: data.column1_text }} />
          ...
          {data.column2_text && (
            <div className='flex flex-col'>
              <div className='content animation-fadeIn' dangerouslySetInnerHTML={{ __html: data.column2_text }} />
          ...
          {data.column3_text && (
            <div className='flex flex-col'>
              <div className='content animation-fadeIn' dangerouslySetInnerHTML={{ __html: data.column3_text }} />
              
              {!!data.column3_btns?.length && (
                <div className='flex flex-wrap gap-4 mt-6 lg:mt-9'>
                  {data.column3_btns.map((btn: any, index: number) => {
                    return (
                      <Button
                        key={index}
                        component={Link}
          ...
          {data.column4_text && (
            <div className='flex flex-col'>
              <div className='content animation-fadeIn' dangerouslySetInnerHTML={{ __html: data.column4_text }} />
              
              {!!data.column4_btns?.length && (
                <div className='flex flex-wrap gap-4 mt-6 lg:mt-9'>
                  {data.column4_btns.map((btn: any, index: number) => {
                    return (
                      <Button
                        key={index}
                        component={Link}
...
export default Section25x25x25x25;
